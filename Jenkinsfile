...
''')
        ...
        stage('triton-origin-image-x86_64-22.4.0') {
            agent {
                node {
                    label joyCommonLabels(image_ver: '22.4.0',
                        pi:'20210826T002459Z')
                }
            }
            when {
                beforeAgent true
                environment name: 'BUILD_22_4_0', value: 'true'
            }
            steps {
                sh('''
export TRACE=1
...
make clean distclean
export ENGBLD_BITS_UPLOAD_IMGAPI=true
make print-BRANCH print-STAMP triton-origin-x86_64-22.4.0-"buildimage bits-upload"
''')
            }
        }
    }
